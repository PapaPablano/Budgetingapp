...
| Two browser tabs open simultaneously show inconsistent data until reload | Accepted, documented — `index.html`'s read-only invariant prevents data loss even in this case, just not staleness. Widened during code review: month-navigation clicks now also persist a full state snapshot (not just data edits), so passive navigation in one tab can clobber a concurrent edit in another — same accepted tradeoff, slightly broader trigger than originally scoped. |
...
| `index.html`'s "Bills" card total can under-state true committed cost when a logged ledger entry overrides a bill's base amount (overage), since the card sums static bill amounts while `remaining` uses the ledger-aware figure | Covered by the already-accepted "Bills card stays bills-only" tradeoff (Key Technical Decisions); the ledger-overage mechanism specifically is now named here rather than left implicit |

## Post-Review Fixes (code review round)

Code review (correctness, testing, maintainability, project-standards, agent-native, learnings, adversarial) surfaced and this session fixed, before shipping:
- **P1 bug**: `computeMonthSummary`'s `awaiting` early return omitted `totalBills`, so `index.html` rendered `"$NaN"` on the Total Bills card whenever income wasn't yet entered for either paycheck (a common state). Fixed by always computing `totalBills` before the awaiting check. Independently flagged by 3 reviewers (correctness, agent-native, adversarial) — strong cross-reviewer agreement. Added as a permanent regression scenario in manual verification.
- **Simplification, rigorously verified**: `computeMonthSummary` originally summed two independently-computed per-cycle verdicts via a new `netOfCycleVerdict` helper, duplicating `computeMonthVerdict`'s pooled-whole-month logic. A reviewer proved via 500,000 randomized trials that the two approaches are mathematically equivalent after the Unit 2 fix (they weren't, before the fix — that's why the "sum of two per-paycheck verdicts" design was chosen over `computeMonthVerdict` in the first place). Verified independently before applying. `computeMonthSummary` now simply calls `computeMonthVerdict` once; `netOfCycleVerdict` was removed and replaced with a general `netOfVerdict` helper. As a side effect, the Unit 2 fix also retroactively fixes the same goal-funding gap in `allocator.html`'s own month-total display, which was already calling `computeMonthVerdict`.
- **Small cleanup**: extracted a shared `unfundedGoalTotal` helper, removing a duplicated formula between `classifyVerdict` and `netOfVerdict`.
- **Defensive guard**: `index.html`'s `renderSummary` now treats a `summary` object missing expected numeric fields (corrupted or legacy data) the same as "awaiting" rather than risking `$NaN`, instead of only guarding against `summary` being entirely absent.
